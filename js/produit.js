let Produits = [
  {
    ...
    nomProduits: "Jumanji",
    imageProduit:
      "https://th.bing.com/th/id/R.4a484e8813e2afe3c516d067887cb28d?rik=D9p3MLRWaprzWA&pid=ImgRaw&r=0",
    descriptionProduits:
      "Partez à l'aventure et riez dans le jeu d'action de Jumanji Survivez au défi ultime pour ceux qui cherchent à laisser leur monde derrière eux Déverrouillez de nouvelles tenues et de nouveaux styles d'armes. Il n'y a jamais deux parties de Jumanji qui se ressemblent !",
    prixProduitHt: 40.95,
    produitQuantite: 1,
    stock:9,
    tva:0.2,
  ...
  },
  {
    ...
    imageProduit:
      "https://cdn.cdkeys.com/700x700/media/catalog/product/b/a/batman-_arkham_collection_xbox_one_eu_cover.jpg",
    descriptionProduits:
      "Chaque titre Batman inclus dans le lot est accompagné de tous les DLC disponibles Faites un voyage à Arkham, et rencontrez certains des méchants les plus emblématiques de la franchise Batman",
    prixProduitHt: 35.99,
    produitQuantite: 1,
    stock:35,
    tva:0.2,
...
function afficherProduit() {
  ...
  Produits.forEach((donnee) => {
    ...
    listeItemLi.innerHTML = `
        ...
        <p>Prix HT :${donnee.prixProduitHt} €</p>
        <p>Disponibilité: ${donnee.stock}</p>
...
function afficherPanier() {
  ...
  panier.forEach((Produit) => {
    ...
    produitPanier +=
     `
        ...
        <p style="color: white">Nom du produit ${Produit.nomProduits}</p>
        ...
        <p style="color: black">Disponibilité: ${Produit.stock}</p>
  ...
  //Recupd de <ul id="panier" et ajoute du li + HTML
  ...
  panier.forEach(supprimer => {
    ...
    btnSupprimer.id = `btn-supprime${supprimer.id}`;
    ...
    //Au clic sur le bouton supprimer-moi
    ...
    let selectQuantitees = document.getElementById(
      `selectQuantites-${supprimer.id}`
    );
    ...
    selectQuantitees.addEventListener("change", () => {
      ...
      document.getElementById(`total-${supprimer.id}`).innerHTML =
        "Prix HT X Quantitées = " + prixMutiplierQuantite + "€";

      //Calcul du montant de TVA  PrixHT * 20%   0.2 = 20 / 100
      let montantTVA = prixMutiplierQuantite * 0.2;
      ...
      document.getElementById(`tva-${supprimer.id}`).innerHTML =
        "Montant de la tva a 20% : " + montantTVA + " €";
      document.getElementById(`ttc-${supprimer.id}`).innerHTML =
        "Montant TTC : " + prixTTC + " €";
    });
  });
